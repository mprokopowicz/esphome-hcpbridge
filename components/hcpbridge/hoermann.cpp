...
uint16_t HoermannGarageEngine::onCurrentStateChanged(TRegister *reg, uint16_t val)
{
  ...
  if (((reg->value & 0xFF00) != (val & 0xFF00)))
  {
    ...
    this->state->rawStatusCode = rawStatus;  // Store for logging in hcpbridge component
    ...
    {
    case 0x1:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> OPENING", rawStatus);
      this->state->setState(HoermannState::State::OPENING);
      break;
    case 0x2:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> CLOSING", rawStatus);
      this->state->setState(HoermannState::State::CLOSING);
      break;
    case 0x20:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> OPEN", rawStatus);
      this->state->setState(HoermannState::State::OPEN);
      break;
    case 0x40:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> CLOSED", rawStatus);
      this->state->setState(HoermannState::State::CLOSED);
      break;
    case 0x80:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> HALFOPEN", rawStatus);
      this->state->setState(HoermannState::State::HALFOPEN);
      break;
    case 0x09:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> MOVE_VENTING", rawStatus);
      this->state->setState(HoermannState::State::MOVE_VENTING);
      break;
    case 0x05:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> MOVE_HALF", rawStatus);
      this->state->setState(HoermannState::State::MOVE_HALF);
      break;
    case 0x0A:
      ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> VENT", rawStatus);
    ...
    case 0x00:
      ...
      {
        ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> VENT (position-based)", rawStatus);
        this->state->setState(HoermannState::State::VENT);
      }
      else
      {
        ESP_LOGI(TAG_HCI, "[STATUS] 0x%02X -> STOPPED", rawStatus);
        this->state->setState(HoermannState::State::STOPPED);
      }
      break;
    default:
      ESP_LOGW(TAG_HCI, "[STATUS] *** UNKNOWN CODE: 0x%02X (decimal=%d) *** - Please report!", rawStatus, rawStatus);
    }
  }
  return val;
}
